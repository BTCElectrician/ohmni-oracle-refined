...
class PanelScheduleProcessor:
    ...
    async def process_panel_schedule(self, file_path: str, gpt_client: AsyncOpenAI) -> Dict:
        ...
        try:
            ...
            # 2) Call begin_analyze_document using the correct parameters for azure-ai-documentintelligence==1.0.0
            ...
            self.logger.debug(
                f"GPT returned a dict with keys: "
                f"{list(structured_json.keys()) if isinstance(structured_json, dict) else 'N/A'}"
            )
